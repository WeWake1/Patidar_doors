 ...
 *
 * ── CMS (Supabase + /admin) ───────────────────────────────────────────────
 * The client manages the catalogue in the custom admin (src/admin/), and the
 * storefront reads it two ways: `npm run catalog:fetch` bakes the published
 * rows into catalog.gen.ts at build time, and `liveCatalog.ts` re-reads them
 * in the browser so a door added this morning appears without a redeploy.
 * `buildCatalogue()` at the bottom merges either of them over this local base.
 * Local data doubles as the seed (scripts/seed-supabase.mjs) and the fallback
 * when no CMS is configured. Setup: docs/admin-setup.md.
 *
...
import type { CmsSections } from './cmsMap'
...
/**
 * How a photo card behaves on hover/scroll. The three the CMS stores — they are
 * CHECK-constrained in Postgres as well as typed here, so a fourth stored value
 * needs a migration (see `widen_products_presentation_still`).
 ...
 */
export type StoredPresentation = 'swing' | 'showcase' | 'still'

/**
 * All three of the above plus `plain`, which is **derived, never stored**:
 * `buildCatalogue` forces it onto every photograph of something that is not a
 * door (see `isDoorProduct`). All three stored values frame the photo as a door
 * — swing and showcase animate it, and even `still` keeps the architrave, which
 * is the whole point of that treatment. A stack of teak logs in a door frame
 * reads as a door. `plain` is the swatch treatment instead: full-bleed 4:3, no
 * frame, no motion, so a photographed board and a generated one sit in one grid.
 *
 * It is derived rather than a fourth column because it is not a choice anyone
 * should have to make — timber is not a door whoever is filling the form in —
 * and because that column is CHECK-constrained, so a stored fourth value would
 * fail the client's *save* with a bare postgres 23514.
 */
export type PhotoPresentation = StoredPresentation | 'plain'
...
/**
 * Local catalogue merged with the CMS: a CMS product with a matching id
 * replaces the local entry and brand-new slugs are appended.
 *
...
/**
 * The catalogue the site renders: the local base, the CMS over it, and three
 * overlays on top.
 *
 * The overlays carry curation that lives in this repo rather than in the CMS,
 * which a wholesale CMS override would otherwise drop:
 *
 ...
 *  3. `presentation: 'plain'` — a photograph of something that is not a door
 *     (see `isDoorProduct`) is shown full-bleed rather than in a door frame.
 *     The CMS has no way to say this: every value its `presentation` column
 *     accepts is a door treatment.
 *
 * It is a function, not a one-off constant, because the same merge now runs
 * twice: once over the build-time snapshot below, and again in the browser
 * over whatever the CMS holds *right now* (`liveCatalog.ts`). Running the
 * second one through anything but this function is how the live site and the
 * built site would start disagreeing about prices and leaf cut-outs.
 */
export function buildCatalogue(cms: Product[]): Product[] {
  const overrides = new Map(cms.map((p) => [p.id, p]))
  const merged: Product[] = [
    ...LOCAL_PRODUCTS.map((p) => overrides.get(p.id) ?? p),
    ...cms.filter((p) => !LOCAL_IDS.has(p.id)),
  ]
  return merged.map((p) => {
    const price = PLACEHOLDER_PRICES[p.id]
    const isDoor = isDoorProduct(p.world, p.sub)
    const leaf =
      isDoor && p.visual.kind === 'photo' && !p.visual.cover.isLeafCrop
        ? leafImageFor(p.id, p.name)
        : undefined
    /* Overlay three: a photograph of something that is not a door is shown as a
       picture, not as a door. The CMS cannot say this — its `presentation`
       column holds only the three door treatments, all of which frame the photo
       in an architrave — so it is decided here, from the world and the section
       the client filed the product under. See `isDoorProduct` for the two-month
       bug this closes. */
    const plain = !isDoor && p.visual.kind === 'photo' && p.visual.presentation !== 'plain'
    if (price === undefined && !leaf && !plain) return p
    const visual =
      leaf || plain
        ? { ...p.visual, ...(leaf ? { leaf } : {}), ...(plain ? { presentation: 'plain' as const } : {}) }
        : undefined
    return {
      ...p,
...
/**
 * The catalogue as it stood when the site was built (catalog.gen.ts, written
 * by `npm run catalog:fetch`).
 *
 * ⚠️ This is a *snapshot*, and components must not read it — it is what the
 * bundle ships with, not what the store is selling this morning. Build-time
 * tools do read it (the sitemap, the geometry checks, the seed), because a
 * build can only describe the catalogue it was given. Everything rendered goes
 * through `useCatalog()` / `getProduct()` / `productsIn()`, which read the live
 * catalogue below and fall back to this until it lands.
 */
export const CATALOGUE_SNAPSHOT: Product[] = buildCatalogue(CMS_PRODUCTS)

/* ═══════════════ the live catalogue ══════════════════════════════════════
 *
 * The site was purely static until 2026-08-30: the only path from /admin to
 * the storefront was `catalog:fetch` → rebuild → redeploy, so a door added in
 * the admin appeared on the site only if someone (or a Supabase→Vercel deploy
 * hook that was never wired up) rebuilt it. The client added products, saw
 * nothing change, and reasonably concluded the admin was broken.
 *
 * `liveCatalog.ts` now re-reads the published catalogue in the browser and
 * publishes it here; the snapshot is what renders until it arrives, so nothing
 * waits on the network to paint and the site is unchanged when Supabase is
 * unreachable or unconfigured.
 */

/** Where the rendered catalogue currently comes from. */
export type CatalogueStatus = 'snapshot' | 'loading' | 'live' | 'error'

let CATALOGUE: Product[] = CATALOGUE_SNAPSHOT
let SECTIONS: CmsSections = {}
let STATUS: CatalogueStatus = 'snapshot'
...
/** Subscribe to catalogue changes (see `useCatalog`). Returns an unsubscribe. */
export function subscribeCatalogue(fn: () => void): () => void {
  listeners.add(fn)
  return () => listeners.delete(fn)
}

/** The catalogue as it is right now — live if it has landed, snapshot if not. */
export function getCatalogue(): Product[] {
  return CATALOGUE
}

export function catalogueStatus(): CatalogueStatus {
  return STATUS
}

/**
 * The client's own section order for a world, once the live catalogue has
 * landed. Undefined until then, so the world page falls back to the order
 * written in worlds.ts.
 */
export function sectionsIn(world: WorldId): string[] | undefined {
  return SECTIONS[world]
}

/** Called by `liveCatalog.ts` — nothing else should touch these three. */
export function setCatalogueStatus(status: CatalogueStatus) {
  if (STATUS === status) return
  STATUS = status
  announce()
}

export function publishCatalogue(cms: Product[], sections: CmsSections) {
  CATALOGUE = buildCatalogue(cms)
  SECTIONS = sections
  STATUS = 'live'
  announce()
}
...
export function productsIn(world: WorldId): Product[] {
  return getCatalogue().filter((p) => p.world === world)
}
...
/**
 * True when a visual fills its card edge to edge instead of standing in the
 * stage's padded box — a generated swatch, or a photograph of one (`plain`).
 *
 * The card, the PDP and the admin's live preview all pick their stage class
 * from this, so a photographed board and a drawn one can never end up framed
 * differently on the same grid.
 */
export function isFullBleedVisual(v: Visual): boolean {
  return v.kind === 'material' || (v.kind === 'photo' && v.presentation === 'plain')
}

/**
 * Is this product a door?
 *
 * ⚠️ **The catalogue used to answer this with `visual.kind`**, and that was a
 * trapdoor with a two-year fuse. A door is `art` or `photo` and board stock is
 * `material` — which held exactly as long as nothing but doors was
 * photographed. It is not a rule about doors; it is a rule about *which
 * products happen to have a picture*. The day the client cropped a photo of
 * teak logs into `/timbers`, that product stopped being `material`, and every
 * door behaviour switched itself on at once: the card became a 3:8 architrave
 * that swings open, the editor offered swing/zoom/still, the cropper asked for
 * four corners "on the door itself" plus a handle-flip and a row of 6′6″–8′
 * door sizes, and the PDP started promising the doorway view. All of it for a
 * log pile.
 *
 * So doorness is asked of the *catalogue position*, which is where the client
 * already states it, and is the one owner of the answer — the admin's controls,
 * the card treatment and `tryState` all read this and cannot disagree.
 *
 *  - `doors`           — always. That is what the world is.
 *  - `timbers`, `ply`  — never. Sawn wood and sheet goods are not doors, no
 *                        matter how good the photograph is.
 *  - `wpc`             — **by section**, because that world genuinely holds
 *                        both: `wpc-cnc-door` and `wpc-digital-veneer-door` are
 *                        doors, `wpc-sheets` is 6–18 mm board. A section whose
 *                        name ends in "Door"/"Doors" holds doors.
 *
 * ⚠️ The WPC rule reads a name the client can edit, so it is deliberately
 * narrow: the *last word*, so "WPC Doors" and "Bathroom Doors" match while
 * "WPC Sheets" and "Door Frames" do not. Renaming "WPC Doors" to something that
 * does not end in the word turns that section's cards into plain pictures and
 * withdraws the doorway view — which is visible immediately in /admin, where
 * the editor says in one line which way it read the section and why.
 */
export function isDoorProduct(world: WorldId, sub: string): boolean {
  if (world === 'doors') return true
  if (world === 'wpc') return /\bdoors?$/i.test(sub.trim())
  return false
}
...
export function tryState(product: Product): TryState {
  if (!isDoorProduct(product.world, product.sub)) return 'no'
...
export function leafOf(product: Product): ProductImage | null {
  ...
  /* A photograph of something that is not a door has nothing to place, however
     it was cropped — and a leaf crop saved before the section was renamed, or
     before this rule existed, would otherwise stand a stack of ply in someone's
     hallway. Asked here rather than only in `tryState` so the route, the PDP
     button and the state cannot drift apart. */
  if (!isDoorProduct(product.world, product.sub)) return null
